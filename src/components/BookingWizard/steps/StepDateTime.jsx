...
function StepDateTime({ formData, setFormData, nextStep }) {
  ...
  const fetchAvailableSlots = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Eroare server: HTTP ${response.status} - ${responseText.substring(
            0,
            100
          )}`
        );
      ...
      if (
        ...
        throw new Error(
          "Serverul returnează o pagină HTML în loc de JSON. Verificați configurarea API-ului și rutele."
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Răspuns invalid de la server. Nu este JSON valid: ${responseText.substring(
            0,
            100
          )}...`
        );
      ...
      if (data.success) {
        ...
        console.log(
          "Slots încărcate cu succes:",
          Object.keys(grouped).length,
          "zile disponibile"
        );
      } else {
        const errorMessage =
          data.error || data.message || "Eroare necunoscută de la server";
      ...
      if (err.message.includes("Failed to fetch")) {
        userMessage =
          "Problemă de conexiune la server. Verificați conexiunea la internet sau încercați mai târziu.";
      } else if (
        err.message.includes("No routes matched") ||
        err.message.includes("Not Found")
      ) {
        userMessage =
          "API-ul nu a fost găsit. Verificați URL-ul sau contactați administratorul.";
      } else if (
        err.message.includes("HTML") ||
        err.message.includes("DOCTYPE")
      ) {
        userMessage =
          "Serverul returnează o pagină web în loc de date. Verificați configurarea API-ului.";
      } else if (
        ...
        userMessage =
          "Problemă de securitate CORS. Contactați administratorul.";
        ...
        userMessage =
          "Eroare internă a serverului (500). Contactați administratorul.";
      }
              ...
              <div className="flex-1">
                ...
                <div className="mt-4 p-3 bg-white/50 rounded-lg">
                  ...
                  <ul className="text-sm text-red-600 mt-2 space-y-1">
                    <li>
                      • Verificați dacă fișierul get_available_slots.php există
                      pe server
                    </li>
                    <li>
                      • Asigurați-vă că API-ul returnează JSON valid, nu HTML
                    </li>
                    <li>• Verificați configurarea CORS pe server</li>
                    <li>
                      • Testați API-ul direct în browser pentru a vedea
                      răspunsul
                    </li>
                  </ul>
                </div>
